 ...
 * Jake Hillard and Eldrick Millares
 * ============================================================================
 */
...
/*
 * Function: void fill_analog_buffer()
 * --------------------------------------------------
 * Her Majesty, The Interrupt Service Routine
 * Attached to TimerThree, every 10us it places a recorded value from the sensor into the analog_buffer.
 */
void fill_analog_buffer() {
  analog_buffer[glob_analog_buffer_index] = analogRead(SENSOR_PIN);
  glob_analog_buffer_index = (glob_analog_buffer_index + 1) % analog_buffer_size;  // Used for wrap-around
}
...
void decode_msg_buffer() {
  ...
  for (int k = 0; k < 4; k++) {
    ...
    if (curr_index > 20) {                                            //Psyche, initial pulse was the EOF pulse. Message now ready to be sent. PERHAPS >= ??
      noInterrupts();
